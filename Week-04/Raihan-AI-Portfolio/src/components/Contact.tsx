...
export function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // No backend wired — simulate a successful send for the demo.
    setSent(true);
    setTimeout(() => {
      setSent(false);
  ...
  return (
    <section id="contact" className="relative px-6 py-28 sm:py-36">
      ...
      <div className="mx-auto max-w-6xl">
        <SectionHeader
          eyebrow="Contact"
          title="Let's build AI products together"
          description="Recruiting for an AI Engineer role, or have a hard problem worth solving? I read every message and reply fast."
          align="center"
        />
            ...
            <div className="glass flex h-full flex-col justify-between rounded-3xl border border-hairline p-7">
              <div>
                ...
                <h3 className="text-2xl font-semibold leading-tight text-white">
                  Open to <span className="text-aurora">AI Engineering</span> roles
                </h3>
                <p className="mt-3 text-sm leading-relaxed text-white/55">
                  Graduating in {PROFILE.graduation} with a B.Tech in Computer Science. Available worldwide,
                  especially interested in teams building LLM applications, agentic systems, and ML infrastructure.
                </p>
              </div>
              ...
              <div className="mt-8 space-y-3">
                <MagneticButton
                  href={SOCIALS.email}
                  className="group w-full rounded-xl bg-gradient-to-r from-accent-blue to-accent-violet px-5 py-3.5 text-sm font-medium text-white shadow-[0_8px_30px_rgba(91,140,255,0.3)]"
                >
                  <Mail className="h-4 w-4" />
                  raihanshaik676@gmail.com
            ...
            >
              <form onSubmit={handleSubmit} className="flex h-full flex-col gap-5">
                ...
                <div className="flex flex-1 flex-col">
                  <label htmlFor="message" className="mb-2 text-xs font-medium uppercase tracking-[0.16em] text-white/45">
                    Message
                  </label>
                  <textarea
                    id="message"
                ...
                <button
                  type="submit"
                  disabled={sent}
                  className="btn-ripple group inline-flex items-center justify-center gap-2 self-start rounded-xl bg-white px-6 py-3 text-sm font-semibold text-ink-950 transition-all duration-300 hover:shadow-[0_8px_30px_rgba(255,255,255,0.2)] disabled:opacity-70"
                >
                  {sent ? (
                    <>
...
function Field({
  ...
  return (
    <div className="flex flex-col">
      <label htmlFor={id} className="mb-2 text-xs font-medium uppercase tracking-[0.16em] text-white/45">
        {label}
      </label>
      <input
        id={id}
